...
FFUploadedFile? convertToFFUploadedFile(String? croppedImageString) {
  if (croppedImageString == null) {
    return null;
  }

  // Parse the JSON string back to a Map
  final Map<String, dynamic> json = jsonDecode(croppedImageString);

  // Create an FFUploadedFile from the parsed JSON
...
String? convertUploadedFileToString(FFUploadedFile? croppedImage) {
  if (croppedImage == null) {
    return null;
  }

  // Convert FFUploadedFile to a JSON string
  return jsonEncode({
    'name': croppedImage.name,
    'bytes': base64Encode(croppedImage.bytes!),
    'height': croppedImage.height,
    'width': croppedImage.width,
    'blurHash': croppedImage.blurHash,
  });
}
...
int generateUserUID(String userId) {
  final alphabet = 'abcdefghijklmnopqrstuvwxyz';
  final uppercaseAlphabet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
  int result = 0;

  for (int i = 0; i < userId.length; i++) {
    final char = userId[i];
    final index = alphabet.indexOf(char);
    final uppercaseIndex = uppercaseAlphabet.indexOf(char);

    if (index != -1) {
      result +=
          (index + 1) * (i + 1); // Multiply by position to make it more unique
    } else if (uppercaseIndex != -1) {
      result += (uppercaseIndex + 25) * (i + 1);
    }
  }
